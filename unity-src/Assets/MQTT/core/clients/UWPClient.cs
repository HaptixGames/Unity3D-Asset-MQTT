...
namespace HG.iot.mqtt.clients
{
    ...
    public class UWPClient: MonoBehaviour, IMqttClient
	{
		...
		public bool Connect (ConnectionOptions options)
		{
			...
			if (options.UseSSL)
			{
				_platform = new uPLibrary.Networking.M2Mqtt.MqttClient(_connectionOptions.Host, _connectionOptions.Port, true, uPLibrary.Networking.M2Mqtt.MqttSslProtocols.TLSv1_0);
				...
				if(options.AcceptInvalidServerCertificate)
				{
					...
					mi.Invoke(_platform, new object[] { _connectionOptions.Host, _connectionOptions.Port, true, uPLibrary.Networking.M2Mqtt.MqttSslProtocols.TLSv1_0, certValidationCallback, null });
				}
			}
			else
				_platform = new uPLibrary.Networking.M2Mqtt.MqttClient(_connectionOptions.Host, _connectionOptions.Port, false, uPLibrary.Networking.M2Mqtt.MqttSslProtocols.None);
	...
	public class UWPClient: MonoBehaviour
	{
